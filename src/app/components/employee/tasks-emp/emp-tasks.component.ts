...
export class EmployeeTasksListComponent implements OnInit {
  ...
  viewTaskDetails(task: Task) {
    ...
    // Refresh the task data when viewing details
    if (task.id) {
      this.refreshSingleTask(task.id);
    }
  }

  refreshSingleTask(taskId: number) {
    if (!taskId) {
      console.error('Task ID is required for refresh');
      return;
    }
    ...
    this.taskService.getTaskById(taskId).subscribe({
      next: (updatedTask) => {
        ...
        if (index !== -1) {
          ...
          // Update selectedTask if it's the one being refreshed
